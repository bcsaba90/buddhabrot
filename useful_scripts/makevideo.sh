...
while getopts "hgdpa:s:w:f:n:c:" opt; do
    case "$opt" in
    h|\?)
        echo "Simple utility to convert an bmp image sequence to a video or a gif. Optionally writing power to the image"
        echo "Converting required a copy of all images, which could take up a lot of space. Currently only bmp is supported and all images have to be the same size. Also image format should follow that outputted by fractal drawing program."
        ...
        echo "   -d         Force delete all bmp files"
...
numberOfBmpsInCurrentFolder=`ls -l *.bmp 2>/dev/null | wc -l`
if [ $numberOfBmpsInCurrentFolder -gt 0 ]; then
		echo "[INFO] There are BMPs in current folder"
		if [ $deleteBmps -eq 1 ]; then
			echo "[WARNING] Deleting all bmp files in this folder"
			rm *.bmp
		else
			echo "[ERROR] Refuse to delete bmp files in current folder. Remove BMP files or run with '-d' to remove them. Abort..."
			exit 1
		fi
fi
animationFrameNameExpression="$directory/$animationName*.bmp"
...
j=0
for i in `ls -la *.bmp | awk '{print $9}' | sort`; do
	mv "$i" "frame_$j.bmp" 2>/dev/null
	let j=$j+1
done
echo "[INFO] Done"
...
if [ $makeGif -eq 0 ]; then
	...
	ffmpeg -hide_banner -y -i frame_%d.bmp -r $framesPerSecond -vb 20M -vcodec mpeg4 -r $framesPerSecond "$resultName" 
else
	resultName="$resultName.gif"
	echo "[INFO] Generating palette"
	ffmpeg -y -i frame_%d.bmp -vf fps=10,scale=320:-1:flags=lanczos,palettegen palette.png -loglevel 0
	echo "[INFO] Creating gif"
	ffmpeg -y -i frame_%d.bmp -i palette.png -r $framesPerSecond -filter_complex "fps=$framesPerSecond,scale=1920:-1:flags=lanczos[x];[x][1:v]paletteuse"  "$resultName"
	rm "palette.png"
fi
echo "[INFO] Done"
